...
class OuestFranceScraper(BaseScraper):
    ...
    def _search_page(self, criteria: SearchCriteria, page: int) -> list[Property]:
        transaction = "louer" if criteria.transaction_type == "rent" else "acheter"
        ...
        if criteria.budget_min:
            params["prix_min"] = str(criteria.budget_min)
        if criteria.budget_max:
            params["prix_max"] = str(criteria.budget_max)
        if criteria.surface_min:
            params["surface_min"] = str(criteria.surface_min)
        ...
        cards = soup.select("[class*='annonce'], .annonce-liste__item, article")
    ...
    def _parse_card(self, card) -> Property | None:
        try:
            ...
            city_el = card.select_one("[class*='city'], [class*='localisation']")
            ...
            m = re.search(r"(\d+)\s*(?:pièce|piece|ch\.)", text)
            if m:
                rooms = int(m.group(1))
            ...
            return Property(
                ...
                rooms=rooms, url=url, source="ouestfrance",
            )
        except Exception as e:
            logger.debug(f"[ouestfrance] Parse error: {e}")
            return None
